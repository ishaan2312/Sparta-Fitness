...
const GymApp = (() => {
  ...
  function renderPicker({ id }, tab) {
    ...
    return `
      ...
      <div class="gym-content">
          ...
          <details class="muscle-group" ${catalogue[muscle].some(e => selected.has(e)) ? 'open' : ''}>
            ...
            <div class="muscle-body">
              ${catalogue[muscle].map(ex => {
                const custom = GymStore.isCustomExercise(ex);
                return `
                <label class="check-row ${selected.has(ex) ? 'checked' : ''} ${custom ? 'custom' : ''}"
                  ${custom ? `data-longpress data-name="${esc(ex)}" data-muscle="${esc(muscle)}" data-tab="${tab}"` : ''}>
                  <input type="checkbox" ${selected.has(ex) ? 'checked' : ''}
                    data-gym="toggleExercise" data-id="${routine.id}" data-name="${esc(ex)}" data-tab="${tab}">
                  <span>${esc(ex)}</span>
                  ${custom ? '<span class="custom-dot" title="Custom - long press to delete"></span>' : ''}
                </label>`;
              }).join('')}
        ...
        `).join('')}
        <p class="hint">Exercises you added yourself are marked with a dot \u2014 long press one to delete it.</p>\n        <button class="gym-btn primary block" data-gym="back" data-tab="${tab}">Done</button>
      </div>`;
  }
  ...
  function renderAnalytics({ vMode, vValue }, tab) {
    const a = GymStore.getAnalytics();
    const lifts = GymStore.getTopLifts(3);
    const filters = GymStore.getVolumeFilters();

    const mode = vMode || 'all';
    const value = vValue || null;
    const series = GymStore.getVolumeSeries({ mode, value });
    const points = series.map(p => ({ label: GymUI.fmtDateShort(p.date), value: p.volume, sub: p.label }));

    const chips = mode === 'muscle' ? filters.muscles : filters.routines;
    const rank = ['1', '2', '3'];
    ...
    return `
      ...
      <div class="gym-content">
        <div class="tile-row">
          <button class="tile tappable" data-gym="nav" data-view="sessions" data-tab="${tab}">
            <span class="tile-label">Total Workouts</span>
            <strong class="tile-value">${a.totalWorkouts}</strong>
            <span class="stat-chevron">\u203a</span>
          </button>

          <div class="tile">
            <span class="tile-label">Streaks</span>
            <div class="tile-split">
              <span><strong>${a.currentStreak}</strong><em>current</em></span>
              <span><strong>${a.longestStreak}</strong><em>longest</em></span>
            </div>
          </div>

          <div class="tile">
            <span class="tile-label">Weight Lifted</span>
            <strong class="tile-value">${a.totalWeight.toLocaleString()}<em>kg</em></strong>
          </div>
        </div>

        <h3 class="section-title">Top Lifts</h3>
        ${lifts.length ? `
          <div class="lift-list">
            ${lifts.map((l, i) => `
              <button class="lift-row" data-gym="openExercise" data-name="${esc(l.exercise)}" data-tab="${tab}">
                <span class="lift-rank">${rank[i]}</span>
                <span class="lift-main">
                  <span class="row-title">${esc(l.exercise)}</span>
                  <span class="row-sub">${l.reps} reps \u00b7 ${esc(GymUI.fmtDateShort(l.date))}</span>
                </span>
                <strong class="lift-weight">${l.weight}<em>kg</em></strong>
              </button>
            `).join('')}
          </div>` : '<p class="empty-note">Log a workout to see your best lifts.</p>'}

        <h3 class="section-title">Weight Lifted per Session</h3>
        <div class="graph-controls">
          <div class="mode-toggle">
            <button class="unit-btn ${mode !== 'muscle' ? 'on' : ''}" data-gym="volMode" data-mode="routine" data-tab="${tab}">By Routine</button>
            <button class="unit-btn ${mode === 'muscle' ? 'on' : ''}" data-gym="volMode" data-mode="muscle" data-tab="${tab}">By Muscle</button>
          </div>
          <div class="filter-row">
            <button class="chip ${mode === 'all' ? 'on' : ''}" data-gym="volFilter" data-tab="${tab}" data-mode="all" data-value="">All</button>
            ${chips.map(c => `
              <button class="chip ${value === c ? 'on' : ''}" data-gym="volFilter" data-tab="${tab}" data-mode="${mode === 'muscle' ? 'muscle' : 'routine'}" data-value="${esc(c)}">${esc(c)}</button>
            `).join('')}
          </div>
        </div>
        ${GymUI.barChart(points, {
          unit: ' kg',
          yTitle: value ? `kg \u00b7 ${esc(value)}` : 'kg lifted',
          empty: value ? `No ${esc(value)} volume logged yet` : 'Log a workout to see your volume'
        })}
  ...
  const deleteCustomExercise = (name, muscle, tab) => {
    const use = GymStore.getExerciseUsage(name);
    const parts = [];
    if (use.routines) parts.push(`${use.routines} routine${use.routines === 1 ? '' : 's'}`);
    if (use.sessions) parts.push(`${use.sessions} past workout${use.sessions === 1 ? '' : 's'}`);
    const warn = parts.length
      ? `"${name}" is used in ${parts.join(' and ')}.\n\nIt will be removed from your routines. Past workouts keep the record.\n\nDelete anyway?`
      : `Delete "${name}"?`;
    if (!confirm(warn)) return;
    GymStore.removeCustomExercise(muscle, name);
    render(tab);
    GymUI.toast('Exercise deleted');
  };
  ...
  const setupLongPress = () => {
    const start = (e) => {
      ...
      pressTimer = setTimeout(() => {
        suppressClick = true;                       // stop the tap from toggling selection
        navigator.vibrate?.(15);
        deleteCustomExercise(row.dataset.name, row.dataset.muscle, row.dataset.tab);
      }, 550);
    };
    ...
    // iOS fires a contextmenu on long press; suppress it on these rows only
  ...
  const ACTIONS = {
    nav: (el) => go(el.dataset.tab, el.dataset.view, {
      ...
      vMode: el.dataset.mode || null, vValue: el.dataset.value || null
    }),
    ...
    volMode: (el) => replace(el.dataset.tab, 'analytics', { vMode: el.dataset.mode, vValue: null }),

    volFilter: (el) => replace(el.dataset.tab, 'analytics', {
      vMode: el.dataset.value ? el.dataset.mode : 'all',
      vValue: el.dataset.value || null
    }),
  ...
  const setupEvents = () => {
    document.addEventListener('click', (e) => {
      if (suppressClick) {            // a long press just handled this touch
        suppressClick = false;
        e.preventDefault();
        e.stopPropagation();
        return;
  ...
  /**
   * Called by the shell whenever a tab becomes visible. Each tab returns to its
   * home screen rather than wherever you happened to leave it — except when a
   * workout is actually in progress, which is the one thing worth resuming.
   */
  const enterTab = (tab) => {
    if (tab === 'workout') {
      if (session && !session.editingId) return resetTo('workout', 'logger');
      return resetTo('workout', 'dashboard');
    }
    if (tab === 'profile') {
      if (session && session.editingId) return render('profile');   // keep an open edit
      return resetTo('profile', 'profile');
    }
  };

  return { init, render, go, back, resetTo, enterTab, hasActiveSession: () => !!session };
})();
