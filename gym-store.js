...
const GymStore = (() => {
  ...
  /** Where an exercise is currently used — shown before deleting it. */
  const getExerciseUsage = (name) => ({
    routines: getRoutines().filter(r => r.exercises.includes(name)).length,
    sessions: getSessions().filter(s => s.entries.some(e => e.exercise === name)).length
  });

  const isCustomExercise = (name) => {
    const custom = getCustom();
    return Object.keys(custom).some(m => custom[m].includes(name));
  };

  /**
   * Remove a custom exercise from the catalogue and from every routine.
   * Past sessions keep the name — rewriting logged history would be worse than
   * losing the muscle mapping for it.
   */
  const removeCustomExercise = (muscle, name) => {
    const custom = getCustom();
    if (custom[muscle]) {
      custom[muscle] = custom[muscle].filter(e => e !== name);
      if (!custom[muscle].length) delete custom[muscle];
      write(K.custom, custom);
    }
    const routines = getRoutines().map(r => ({ ...r, exercises: r.exercises.filter(e => e !== name) }));
  ...
  /** Heaviest single sets ever, best first, one entry per exercise. */
  const getTopLifts = (limit = 3) => {
    const best = {};
    getSessions().forEach(s => s.entries.forEach(e => {
      e.sets.forEach(st => {
        if (st.weight <= 0) return;
        if (!best[e.exercise] || st.weight > best[e.exercise].weight) {
          best[e.exercise] = { exercise: e.exercise, weight: st.weight, reps: st.reps, date: s.date };
        }
      });
    }));
  ...
  /**
   * Volume per training day for the volume graph.
   * mode 'all' | 'routine' | 'muscle'. Only days containing qualifying work are
   * returned, so filtering by a muscle collapses the gaps instead of leaving
   * empty space where you trained something else.
   */
  const getVolumeSeries = ({ mode = 'all', value = null } = {}) => {
    const out = [];
    getSessions().slice().sort(byTime).forEach(s => {
      let volume = 0;
      if (mode === 'routine') {
        if (s.routineName !== value) return;
        s.entries.forEach(e => e.sets.forEach(st => { volume += st.weight * st.reps; }));
      } else if (mode === 'muscle') {
        s.entries.forEach(e => {
          if (muscleOf(e.exercise) !== value) return;
          e.sets.forEach(st => { volume += st.weight * st.reps; });
        });
      } else {
        s.entries.forEach(e => e.sets.forEach(st => { volume += st.weight * st.reps; }));
      }
      if (volume > 0) out.push({ date: s.date, volume: Math.round(volume), label: s.routineName || '' });
    });
    return out;
  };

  /** Routine names and muscles that actually appear in logged history. */
  const getVolumeFilters = () => {
    const sessions = getSessions();
    const routines = [...new Set(sessions.map(s => s.routineName).filter(Boolean))];
    const muscles = [...new Set(sessions.flatMap(s => s.entries.map(e => muscleOf(e.exercise))))];
    return { routines, muscles: muscles.filter(m => m !== 'Other') };
  };
